...
volatile send_state_t tx_event = DO_NOTHING;
...
static unsigned char ResendACK[FRAME_SZ_UP];
...
/* 			MakeFrame(&Tx[idx], frame, 20);
		...
		case SEND_ACK:
			...
			SymbolsToUpsampling(frame, coeffs, DAC_samples, FRAME_SZ_UP, FRAME_SZ);
			tx_event = DO_NOTHING;
		    memcpy(ResendACK,  DAC_samples, FRAME_SZ_UP);
		    XTmrCtr_Start(&TimerCounterInst_0, TIMER_CNTR_0);
			XUartLite_Send(&UART_Inst_Ptr_2, DAC_samples, FRAME_SZ_UP);
			while (TotalSentCount_2 != FRAME_SZ_UP);
...
/* 			tx_event = WAIT_FOR_ACK;
		...
		case RESEND_ACK:
			...
			tx_event = DO_NOTHING;
			XTmrCtr_Start(&TimerCounterInst_0, TIMER_CNTR_0);
			XUartLite_Send(&UART_Inst_Ptr_2, ResendACK, FRAME_SZ_UP);
			while (TotalSentCount_2 != FRAME_SZ_UP);
			break;
